...
- **The envelope property for unibranch projective varieties**
  ...
  > **Abstract.** Let $X$ be an integral unibranch projective variety over an algebraically closed field $k$, endowed with the trivial absolute value, and let $\theta\in N^1(X)_{\R}$ be a numerical class. We prove that for every nonempty, bounded-above family $(\varphi_i)\subseteq\PSH(X,\theta)$ of $\theta$-plurisubharmonic functions on the Berkovich analytification $X^{\an}$, the regularized supremum $\sups_i\varphi_i$ is again $\theta$-plurisubharmonic. This establishes the envelope conjecture of Boucksom--Jonsson.
