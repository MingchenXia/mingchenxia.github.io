...
- [A Counterexample to Positive Lower Hermitian Monge–Ampère Mass](Papers/volinf0_vibe.pdf)

  > **Abstract.** We construct a compact complex threefold $X$, a Hermitian form $\omega$ on $X$, and smooth strictly $\omega$-plurisubharmonic functions $\varphi_j$ such that $\int_X(\omega+\mathrm{dd}^c\varphi_j)^3\to 0$. Thus the lower Monge–Ampère mass $v_-(\omega)$ can vanish. The construction combines a Gauduchon-cone separation argument with accumulating deck translates of a local hypersurface, bounded-degree averages of rational curves, and the Hermitian Calabi–Yau theorem.
